...
import { assertPackTargetWritable, writePackDraft } from '../ingest/pack-writer.js';
...
async function pickDrafter(
  ...
  // With no free-memory measurement every verdict is unknown, so honor
  // the documented default (the largest local model) rather than
  // silently inverting to the weakest drafter. Only when fit was
  // actually measured and nothing fits does the smallest make sense.
  const freeUnknown = freeMib === null;
  const fallback = [...assessed].sort((a, b) =>
    freeUnknown
      ? b.descriptor.sizeBytes - a.descriptor.sizeBytes
      : a.descriptor.sizeBytes - b.descriptor.sizeBytes,
  )[0];
  if (fallback === undefined) {
    throw new Error('no local model to draft with; pull one (ollama pull gemma3:4b) or pass --model');
  }
  return fallback.descriptor.name;
}

/**
 * Saves the drafting model's raw response next to the intended pack
 * directory so a failed ingest never discards the generation.
 *
 * @param dir - The absolute pack directory the draft was meant for.
 * @param output - The raw drafting response.
 * @returns The path written: <dir>-draft-response.txt.
 * @throws Only when even the parent directory cannot be created.
 */
function saveDraftResponse(dir: string, output: string): string {
  const path = `${dir}-draft-response.txt`;
...
function summarize(draft: PackDraft, dir: string, provenance: PackProvenance): string {
  const paramNote =
    draft.scorer === 'exact-label' && draft.scorerParams['labels'] !== undefined
      ? `labels: ${JSON.stringify(draft.scorerParams['labels'])}`
      : draft.scorer === 'field-f1' && draft.scorerParams['key_fields'] !== undefined
 ...
 * @throws When the source is unreadable, the target directory is not
 *   empty (checked before any generation when --dir names it), no
 *   drafting model exists, or the draft holds no salvageable JSON after
 *   all repair rounds; each error says what to do, and a draft that
 *   cannot be written is kept at <dir>-draft-response.txt, next to the
 *   intended directory, for inspection.
 */
export async function ingestCommand(options: IngestCommandOptions): Promise<string> {
  ...
  if (options.dir !== undefined) {
    // Fail on a bad target now, before minutes of drafting inference.
    assertPackTargetWritable(options.dir);
  }
  ...
  if (salvaged === null) {
    const responsePath = saveDraftResponse(dir, lastOutput);
    throw new Error(
      `the drafting model never produced JSON; its last response is at ${responsePath}. Retry with a stronger model: quantproof ingest ${options.source} --model <name>`,
    );
  }
  ...
  let written;
  try {
    written = writePackDraft(dir, salvaged, provenance);
  } catch (err) {
    // A collision on the derived directory surfaces only here, after
    // the generation was spent; keep the draft instead of discarding it.
    const responsePath = saveDraftResponse(dir, lastOutput);
    const reason = err instanceof Error ? err.message : String(err);
    throw new Error(
      `${reason}; the finished draft response was saved to ${responsePath}, so fix the directory and rerun: quantproof ingest ${options.source} <new-dir>`,
      { cause: err },
    );
  }
