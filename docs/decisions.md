...
- Case study (2026-07-17) ran the Ollama ladder as gemma3:1b plus gemma4:e4b only: the Rapid-MLX server keeps ~26 GiB resident and cannot be unloaded, so larger Ollama models would have contended for unified memory and corrupted the latency numbers; the study configs (quantproof.yaml, quantproof.rapid-mlx.yaml) ship in the repo for reproduction.
- Rapid-MLX backend (2026-07-17): plain HTTP against the OpenAI-compatible API, no SDK, behavior pinned to a live 0.6.0 instance. The server holds one resident model for its lifetime, so load verifies /health and unload is a no-op; ensureModelAvailable cannot pull, its error names the restart command. Version comes from /openapi.json info.
- Rapid-MLX prompt cache is cleared before every generation (/v1/cache/clear): the live server replayed a repeated identical request from cache, which would let reps 2 and 3 measure cache latency instead of inference. A clear failure degrades to a recorded caveat in requestOptions, never a silent wrong number.
- Rapid-MLX memory comes from the server's own Metal accounting at /v1/status, polled per candidate with the peak tracked over the probe window only: MLX Metal buffers do not appear in process RSS (live evidence: RSS 1.3 GiB vs Metal active 26.0 GiB for the served 30B), so the unified-memory probe would report garbage for this backend.
- Rapid-MLX 0.6.0 with continuous batching did not produce identical outputs for identical seeded temperature-0 requests in live probing (two runs of the same prompt differed); the existing nondet flag and quality spread carry that honestly when it fires in a sweep, and methodology documents it as a backend property, not a quantproof bug.
- Unified-memory measurement (2026-07-17): on Apple Silicon the probe samples the summed RSS of the backend's processes via ps (async spawn so TTFT timing is never blocked), validated live against Ollama's own accounting (runner RSS 14.4 GiB vs /api/ps 14.1 GiB for an 11 GB model with KV); the fit budget is 75% of physical memory (Metal's working-set convention) minus resident backend memory. Chosen over vm_stat system-wide counters because per-process RSS is delta-clean and works for any local backend. nvidia-smi still wins when present; selection lives in one place (telemetry/probe-select).
- Report wording generalized from VRAM to memory (peak memory, memory not measured) because Mac numbers are unified memory, not VRAM; database column names (peakVramMib and friends) stay as-is, a rename would churn the schema for zero user value.
...
- Ingest (2026-07-17): the drafting model proposes the pack as one JSON object; parseDraft is the trust boundary (strict, all errors in one pass, scorer-aware checks like expected-in-labels) and gets up to two repair rounds with the errors fed back. The drafter defaults to the largest local model whose fit verdict is "fits" at the drafting context (16384), reusing the planner's assessment; explicit --model overrides.
